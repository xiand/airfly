...
void DemoFatFS(void)
{
	...
	uint8_t ucData[1024];
	uint8_t 		ucKeyCode = 0;
	SD_CardInfo  mySdCardInfo;
	cmd = SD_GetCardInfo(&mySdCardInfo);
	printf("the SD_GetCardInfo cmd = %X\r\n", cmd);
	/* 打印命令列表， 用户可以通过串口操作指令 */
	//DispMenu();
	for(i = 0; i < 1024;i++)
		data[i] =12;
		...
		if (ucKeyCode != KEY_NONE)
		{
			switch (ucKeyCode)
			{
				case KEY_1_SHORTPRESS:
					printf("按键短按 ViewRootDir\r\n");
					//ViewRootDir();
				//cmd = SD_WriteBlock(data, 1000,512);
				//printf("cmd = %d\r\n",cmd);
				printf("the CardCapacity of SDCard = %X\r\n",mySdCardInfo.CardCapacity);
					break;

				case KEY_1_LONGPRESS:
					printf("按键长按 CreateNewFile\r\n");
					//CreateNewFile();
					break;

				case KEY_1_DOUBLEPRESS:
					printf("按键双击 ReadFileData\r\n");
					printf("the CardBlockSize of SDCard = %X\r\n",mySdCardInfo.CardBlockSize);
					#if 0
				cmd = SD_ReadBlock(ucData,1000, 512);
				printf("sd_ReadSingleBlock cmd = %d\r\n",cmd);
				for(i = 0; i < 1000;i++)
				{
						printf("%d ",ucData[i]);
				}
